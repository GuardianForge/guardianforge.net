          ...
          <div className="config-modal-left">
            {item?.getPerkSockets() && (
              <div className="perks-row">
                ...
                <div className="perks">
                  {item?.getPerkSockets()?.map((socket: Socket, idx: number) => (
                    <div className="perk-column" key={`socket-${socket.position}-${idx}`}>
                      {socket.availablePlugs ? socket.availablePlugs?.map((plug: SocketItem) => (
                        <img onClick={() => setEquippedPlug(socket, plug)}
                          className={`available ${socket.equippedPlug?._meta?.manifestDefinition?.hash === plug._meta?.manifestDefinition?.hash ? 'equipped' : ""}`}
                          src={plug.iconUrl}
                          key={`socket-${socket.position}-plug-${plug._meta?.manifestDefinition.hash}`}
                          alt="Plug Icon" />
                      )) : (
                        <img className="equipped" src={socket.equippedPlug?.iconUrl} alt="Plug Icon"/>
                      )}
                    </div>
                  ))}
                </div>
                    ...
                    <>
                      ...
                      {!socket.isItemTierSocket && socket.equippedPlug && socket.equippedPlug.iconUrl !== "https://www.bungie.netundefined" && (
                        <div className="mod-socket">
                          <img onClick={() => showModDrawer(socket)} src={socket.equippedPlug?.iconUrl} alt="Plug Icon" />
                        </div>
                      )}
                    </>
                  ))}
            ...
            {item?.getIntrinsicTraits()?.map((el: SocketItem, idx: number) => (
              <div className="intrinsic-row" key={`intrinsic-${idx}`}>
                <div className="row-header">Intrinsic Traits</div>
                <div className="intrinsic-trait">
                  <img src={el.iconUrl} alt="Trait Icon" />
                  <div>
