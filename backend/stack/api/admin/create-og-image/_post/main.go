...
type PostCreateOgImageRequest struct {
	BuildId string `json:"buildId"`
}

type PostCreateOgImageResponse struct {
...
func handler(request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	if !utils.IsAdmin(membershipId) {
		return utils.UnauthorizedResponse(nil)
	}

	var req PostCreateOgImageRequest
	json.Unmarshal([]byte(request.Body), &req)

	bucketName := os.Getenv("BUCKET_NAME")
	key := fmt.Sprintf("builds/%v.json", req.BuildId)
	log.Println(bucketName, key)
	err = createImage(bucketName, key)
	if err != nil {
		return utils.ErrorResponse(err, "(handler) createImage")
	}

	ogImageUrl := fmt.Sprintf("https://%v.s3.amazonaws.com/og/%v.png", bucketName, req.BuildId)

	jbytes, err := json.Marshal(ogImageUrl)
	if err != nil {
